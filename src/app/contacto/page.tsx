...
const SERVICIOS = [
  { valor: "Análisis clínicos", icono: IconDroplet, ayuda: "Sangre, orina y perfiles de laboratorio" },
  { valor: "Ultrasonido", icono: IconScan, ayuda: "Generales y obstétricos" },
  { valor: "Tomografía", icono: IconXRay, ayuda: "Disponible las 24 horas" },
  { valor: "Radiología", icono: IconFocus, ayuda: "Rayos X, disponible las 24 horas" },
  { valor: "Mastografía", icono: IconHeartPulse, ayuda: "Detección oportuna" },
  { valor: "Convenio empresarial", icono: IconUsers, ayuda: "Exámenes para tu empresa" },
  { valor: "Estudio específico", icono: IconClipboard, ayuda: "Ya elegí un estudio del catálogo" },
];
...
function ContactoBody() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!formData.sinCita && !formData.fechaCita) {
      setFormError("Indica el día en que quieres asistir, o marca “Aún no sé qué día” para que te ayudemos a elegirlo.");
      return;
    }
    setFormError("");
    setEnviando(true);
    const message = [
      ...
      formData.sinCita ? "Aún no tengo día definido; solicito apoyo para elegirlo." : `Día de la cita: ${formData.fechaCita}`,
      !formData.sinCita && formData.horaCita && `Hora de la cita: ${formData.horaCita}`,
              ...
              {enviado ? (
                ...
                <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                  <div className="form-field md:col-span-2">
                    <p className="text-xs font-bold uppercase tracking-wider text-gray-400">Solo 3 pasos</p>
                    <p className="mt-1 text-sm text-gray-600">Elige tu estudio, el día que quieres asistir y déjanos tus datos. Te confirmamos por WhatsApp.</p>
                  </div>
                  ...
                  <div className="form-field md:col-span-2">
                    <p className="mb-3 flex items-center gap-2 text-xs font-black uppercase tracking-wider text-lad-black">
                      <span className="flex h-5 w-5 items-center justify-center rounded-full bg-lad-red text-[10px] text-white">1</span>
                      ¿Qué necesitas agendar?
                    </p>
                    <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3">
                      {SERVICIOS.map(({ valor, icono: Icono, ayuda }) => {
                        const activo = formData.servicio === valor;
                        return (
                          <button
                            key={valor}
                            type="button"
                            onClick={() => setFormData((prev) => ({ ...prev, servicio: valor }))}
                            aria-pressed={activo}
                            className={`flex flex-col items-start gap-2 rounded-xl border p-3 text-left transition ${
                              activo ? "border-lad-red bg-lad-red/5 shadow-sm" : "border-gray-200 hover:border-lad-red/40"
                            }`}
                          >
                            <span className={`flex h-9 w-9 items-center justify-center rounded-full ${activo ? "bg-lad-red text-white" : "bg-lad-gray-light text-lad-red"}`}>
                              <Icono className="h-5 w-5" />
                            </span>
                            <span className="text-xs font-bold leading-tight text-lad-black">{valor}</span>
                            <span className="text-[11px] leading-snug text-gray-500">{ayuda}</span>
                          </button>
                        );
                      })}
                    </div>
                  ...
                  {/* Paso 2: día y hora */}
                  <div className="form-field border-2 border-lad-red/20 bg-lad-red/5 p-4 md:col-span-2">
                    <p className="mb-3 flex items-center gap-2 text-xs font-black uppercase tracking-wider text-lad-black">
                      <span className="flex h-5 w-5 items-center justify-center rounded-full bg-lad-red text-[10px] text-white">2</span>
                      ¿Qué día quieres asistir?
                    </p>
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      <div>
                        <label htmlFor="fechaCita" className="mb-2 block text-xs font-bold uppercase tracking-wider text-gray-500">Día de tu cita</label>
                        <input
                          id="fechaCita"
                          className="w-full border border-gray-200 bg-white p-3 disabled:bg-gray-100 disabled:text-gray-400"
                          name="fechaCita"
                          type="date"
                          value={formData.fechaCita}
                          onChange={handleChange}
                          disabled={formData.sinCita}
                          required={!formData.sinCita}
                        />
                      </div>
                      <div>
                        <label htmlFor="horaCita" className="mb-2 block text-xs font-bold uppercase tracking-wider text-gray-500">Hora (opcional)</label>
                        <input
                          id="horaCita"
                          className="w-full border border-gray-200 bg-white p-3 disabled:bg-gray-100 disabled:text-gray-400"
                          name="horaCita"
                    ...
                    <label className="mt-3 flex items-center gap-2 text-xs font-semibold text-lad-black">
                      <input type="checkbox" name="sinCita" checked={formData.sinCita} onChange={handleNoCitaChange} className="h-4 w-4 accent-lad-red" />
                      Aún no sé qué día; que me ayuden a elegirlo.
                    </label>
                  </div>

                  {/* Paso 3: tus datos */}
                  <div className="form-field md:col-span-2">
                    <p className="flex items-center gap-2 text-xs font-black uppercase tracking-wider text-lad-black">
                      <span className="flex h-5 w-5 items-center justify-center rounded-full bg-lad-red text-[10px] text-white">3</span>
                      Tus datos
                    </p>
                  </div>
                  ...
                  <textarea className="form-field min-h-36 border border-gray-200 p-3 md:col-span-2" name="mensaje" placeholder="Mensaje o comentarios adicionales (opcional)" value={formData.mensaje} onChange={handleChange} />
        ...
        <div className="mx-auto grid max-w-5xl grid-cols-1 gap-5 sm:grid-cols-2">
          {LAD_SUCURSALES.map((sucursal, index) => (
            <ScrollReveal key={sucursal.slug} delay={index * 0.06}>
              <div className="flex h-full flex-col rounded-2xl border border-gray-200 bg-white transition-colors hover:border-lad-red/40">
                <div className="flex items-start gap-3 px-5 pt-4">
                  <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-lad-red/10 text-lad-red">
